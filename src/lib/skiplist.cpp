...
{
  _CSSL_SkipList *slist = reinterpret_cast<_CSSL_SkipList *>(malloc(sizeof(*slist)));

  slist->max_level = maxLevel;
  slist->num_elements = 0;
  slist->head = reinterpret_cast<_CSSL_DataNode *>(newNode(0));
  slist->tail = slist->head;
  slist->skip = skip > 1 ? skip : 2;
  slist->sqr_skip = slist->skip * slist->skip;
  slist->items_per_level = reinterpret_cast<uint32_t *>(malloc(sizeof(uint32_t) * maxLevel));
  slist->starts_of_flanes = reinterpret_cast<uint32_t *>(malloc(sizeof(uint32_t) * maxLevel));
  slist->flane_items = reinterpret_cast<uint32_t *>(malloc(sizeof(uint32_t) * maxLevel));
...
{
  _CSSL_ProxyNode *proxy = reinterpret_cast<_CSSL_ProxyNode *>(malloc(sizeof(*proxy)));
...
void insertElement(_CSSL_SkipList *slist, uint32_t key)
{
  ...
  if (slist->num_elements %
          (TOP_LANE_BLOCK * ((uint32_t)pow(slist->skip, slist->max_level))) ==
      0)
...
void buildFastLanes(_CSSL_SkipList *slist)
{
  ...
  for (int8_t level = slist->max_level - 2; level >= 0; level--)
  {
    ...
    printf("%i:%i", level, flane_size);
  }

  slist->flanes = reinterpret_cast<uint32_t *>(malloc(sizeof(uint32_t) * flane_size));
  slist->flane_pointers = reinterpret_cast<_CSSL_ProxyNode **>(malloc(sizeof(_CSSL_ProxyNode *) * slist->items_per_level[0]));
...
void resizeFastLanes(_CSSL_SkipList *slist)
{
  uint32_t new_size = slist->items_per_level[slist->max_level - 1] +
                      TOP_LANE_BLOCK;
  uint32_t *level_items = reinterpret_cast<uint32_t *>(malloc(sizeof(uint32_t) * slist->max_level));
  uint32_t *level_starts = reinterpret_cast<uint32_t *>(malloc(sizeof(uint32_t) * slist->max_level));
  ...
  uint32_t *new_flanes = reinterpret_cast<uint32_t *>(malloc(sizeof(uint32_t) * new_size));
  _CSSL_ProxyNode **new_fpointers = reinterpret_cast<_CSSL_ProxyNode **>(malloc(sizeof(_CSSL_ProxyNode *) * level_items[0]));
...
uint32_t searchElementPrefetched(_CSSL_SkipList *slist, uint32_t key)
{
  uint32_t curPos = 0;
  uint32_t first = 0;
  uint32_t last = slist->items_per_level[slist->max_level - 1] - 1;
  ...
  //   curPos = slist->starts_of_flanes[level - 1] + rPos * slist->skip;
  // }
  for (level = slist->max_level - 1; level >= 4; level--)
  {
    uint32_t rPos = curPos - slist->starts_of_flanes[level];
    uint32_t prefetchPos = slist->starts_of_flanes[(level > 0) ? level - 1 : 0] + ((rPos)*slist->skip) + 1;
    uint32_t prefetchPos2lvl = slist->starts_of_flanes[(level > 1) ? level - 2 : 0] + ((rPos)*slist->sqr_skip) + 1;
    //_mm_prefetch(slist->flanes + prefetchPos, _MM_HINT_T0);
    //_mm_prefetch(reinterpret_cast<void *>(slist->flanes + prefetchPos), _MM_HINT_T0);
    // printf("Level: %i\n", level);

    // printf("prefetchPos2lvl: %i\n", prefetchPos2lvl);
    // printf("currPos: %i\n", curPos);
    while (rPos < slist->items_per_level[level] &&
           key >= slist->flanes[++curPos])
    {
      prefetchPos += slist->skip;
      prefetchPos2lvl += slist->sqr_skip;
      // printf("PrefetchPos2lvl loop: %i\n", prefetchPos2lvl);
      // printf("currPos loop: %i\n", curPos);
  ...
  for (; level >= 0; level--)
  {
    ...
    uint32_t prefetchPos2lvl = slist->starts_of_flanes[(level > 1) ? level - 2 : 0] + ((rPos)*slist->sqr_skip) + 1;
    //_mm_prefetch(slist->flanes + prefetchPos, _MM_HINT_T0);
    prefetch<T0>(slist->flanes + prefetchPos);
    //_mm_prefetch(reinterpret_cast<void *>(slist->flanes + prefetchPos), _MM_HINT_T0);
    // printf("Level: %i\n", level);
  ...
  for (uint8_t i = 1; i < slist->skip; i++)
  {
    //_mm_prefetch(proxy->keys + std::max(i + 1, static_cast<int>(slist->skip)), _MM_HINT_NTA);
    if (proxy->keys[i] == key)
      return key;
  }
  return key;
...
#if defined(AARCH64)
  // TODO: handle searchRange on AARCH64
  throw std::runtime_error("SearchRange not implemented on AARCH64!");
  return result;
#else
...
{
  _CSSL_DataNode *node = reinterpret_cast<_CSSL_DataNode *>(malloc(sizeof(*node)));
  node->key = key;
  node->next = NULL;
