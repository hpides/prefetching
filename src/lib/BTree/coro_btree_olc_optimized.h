...
namespace btreeolc::coro_optimized
{
    ...
    template <const uint64_t pageSize, const u_int64_t cacheLineSize>
    ...
    struct BTree
    {
        using optimized_task_type = root_task;
        ...
        root_task insert(Key k, Value v)
        {
            int restartCount = 0;
        restart:
            if (restartCount++)
        ...
        root_task lookup(Key k, Value &result)
        {
            int restartCount = 0;
        restart:
            if (restartCount++)
