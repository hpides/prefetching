...
namespace btreeolc::coro_lines
{
    ...
    inline thread_local tcalloc co_allocator;
    ...
    public:
        ...
        struct promise_type
        {
            ...
            void *operator new(size_t sz) { return co_allocator.alloc(sz); }
            void operator delete(void *p, size_t sz) { co_allocator.free(p, sz); }
    ...
    struct NodeBase : public OptLock
    {
        ...
        void prefetch_address(void *addr)
        {
            SWPrefetcher::prefetch<0U, 1U, SWPrefetcher::Target::ALL>(addr);
        }
    ...
    struct BTreeLeaf : public BTreeLeafBase<pageSize, cacheLineSize>
    {
        ...
        Task co_lowerBound(Key k, unsigned &result)
        {
            unsigned lower = 0;
    ...
    struct BTreeInner : public BTreeInnerBase<pageSize, cacheLineSize>
    {
        ...
        Task co_lowerBound(Key k, unsigned &result)
        {
            ...
            {
                unsigned mid = ((upper - lower) / 2) + lower;
                this->prefetch_address(std::addressof(keys[mid]));
    ...
    struct BTree
    {
        ...
        restart:
            ...
            while (node->type == PageType::BTreeInner)
            {
                 ...
                 */
                unsigned res;
                auto c = inner->co_lowerBound(k, res);
                while (!c.is_done())
                {
                    c.resume();
                    co_await std::suspend_always{};
            ...
            unsigned res;
            auto c = leaf->co_lowerBound(k, res);
            while (!c.is_done())
            {
                c.resume();
                co_await std::suspend_always{};
