...
#define MALLOC(SZ) numa_alloc->alloc_func(SZ + RELATION_PADDING, CACHE_LINE_SIZE) /*malloc(SZ+RELATION_PADDING)*/
// #define FREE(X, SZ) free(X)
#define FREE(X, SZ) numa_alloc->free_func(X, SZ)
 ...
 * relation must have been allocated
 */
void random_gen(relation_t *rel, const int32_t maxid)
{
    uint32_t i;
...
int create_relation_pk(relation_t *relation, int32_t num_tuples, NumaAllocator *numa_alloc)
{
    check_seed();

    relation->num_tuples = num_tuples;
    relation->tuples = (tuple_t *)MALLOC(relation->num_tuples * sizeof(tuple_t));
    // relation->tuples = (tuple_t *)alloc(relation->num_tuples * sizeof(tuple_t) + RELATION_PADDING, 8);
...
int parallel_create_relation_pk(relation_t *relation, int32_t num_tuples,
                                uint32_t nthreads, NumaAllocator *numa_alloc)
{
    uint32_t i, rv;
    uint32_t offset = 0;
...
int numa_localize(tuple_t *relation, int32_t num_tuples, uint32_t nthreads)
{
    uint32_t i, rv;
    uint32_t offset = 0;
...
int create_relation_fk(relation_t *relation, int32_t num_tuples, const int32_t maxid, NumaAllocator *numa_alloc)
{
...
int create_relation_fk_from_pk(relation_t *fkrel, relation_t *pkrel,
                               int32_t num_tuples, NumaAllocator *numa_alloc)
{
    int i, iters, remainder;

    fkrel->tuples = MALLOC(num_tuples * sizeof(tuple_t));
...
int create_relation_nonunique(relation_t *relation, int32_t num_tuples,
                              const int32_t maxid, NumaAllocator *numa_alloc)
{
    check_seed();
...
int create_relation_zipf(relation_t *relation, int32_t num_tuples,
                         const int32_t maxid, const double zipf_param, NumaAllocator *numa_alloc)
{
    check_seed();
...
void delete_relation(relation_t *rel, NumaAllocator *numa_alloc)
{
    /* clean up */
