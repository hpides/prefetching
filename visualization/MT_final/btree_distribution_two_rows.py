...
def plot_btree_benchmark(distribution):
    ...
    for reliability, (unique_id, group_id) in [
        ...
        i += 1
        ax_top = axes[i]
        ax_bottom = axes[i+ number_columns]
        axes_pair = [ax_top, ax_bottom]
        for ax in axes_pair:
            ax.set_xscale("log")
            ax.grid(axis="y")
        ...
        for bsearch_variant in unique_bsearch_variants:
            ...
            x += 2.2
            for row, ax in enumerate(axes_pair):
                ...
                ax.plot(
                    unique_tree_node_sizes[: len(runtimes[row])],
                    runtimes[row],
        ...
        for row, ax in enumerate(axes_pair):
            ...
            if i > 0 and node_groups_filtered[i - 1][1] == group_id:
                ax.sharey(axes[i - 1 + row * number_columns])
                ax.tick_params(labelleft=False)
            else:
                if i > 0:
                    lower = 0.14 if row == 1 else 0.595
                    upper = lower + .32
                    draw_line_left_to_plot_new(
                        fig,
                        ax,
                        x_plot_offsets[row] + 0.015,
                        GROUPS_MEM_TYPE[node_groups_filtered[i - 1][1]],
                        GROUPS_MEM_TYPE[node_groups_filtered[i][1]],
                        lower=lower,
                        upper=upper,
                    )
                    x_plot_offsets[row] += GROUPS_SPACING_SEPARATOR
            move_axis_right(ax, x_plot_offsets[row])
            if i == 5 or i == 6:
                x_plot_offsets[row] += increase_width(ax)
    ...
    for ax, long in zip(axes, [False, False, False, False, False, True, True, False, False, False, False, False, False, True, True, False]):
    ...
    axes[number_columns].set_ylabel("Runtime [s]", fontsize=LABEL_FONT_SIZE)
    fig.text(
        0.5,
        0.53,
        "Node Size [KiB]",
    ...
    axes[number_columns].set_yticks([5, 10, 15, 20, 25], [5, 10, 15, 20, 25])
    axes[number_columns+6].set_yticks([10, 20, 30, 40], [10, 20, 30, 40])
    plt.savefig(f"{MT_FINAL_FIGURES_DIR}/btree_diff_node_sizes_two_rows.pdf")
    #plt.show()
