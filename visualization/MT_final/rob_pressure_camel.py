...
DATA = "rob_pressure_camel_finaleeee"  # "rob_pressure_nops_no_jemalloc" ,"rob_pressure_ca_alloc"
...
def add_fancy_labels_and_legend(fig, axes, ylabel):
    fig.text(
        0.5,
        0.005,
        "Number of Executed Hash Functions per Access",
        ha="center",
        va="bottom",
        fontsize=30,
    )
...
def plot_rob_pressure_runtimes():
    ...
    fig.savefig(f"{MT_FINAL_FIGURES_DIR}/rob_pressure_camel.pdf")
    # plt.show()
...
def plot_rob_pressure():
    ...
    fig.savefig(f"{MT_FINAL_FIGURES_DIR}/rob_pressure_camel_speedup.pdf")
    # plt.show()
