...
def plot_lfb_full_behavior_vary_locality():
    ...
    for unique_locality in LOCALITIES:
        ...
        plt.tight_layout(rect=[0.025, 0.03, 1, 0.94])
        plt.subplots_adjust(hspace=0.6)  # Increase the space between subplots
        plt.subplots_adjust(wspace=0.15)  # Increase the space between subplots
        # move_axis_right(axes[6], 0.25)
        axes[6].set_xlabel(" ")
        axes[7].set_xlabel(" ")
        axes[6].set_ylabel(" ")
        axes[7].set_ylabel(" ")
        axes[6].set_title("ARM1-Weak")
        axes[7].set_title("ARM1-Strong")
        for ax in axes:
            ax.legend([], [], frameon=False)
            ax.grid(True)
            ax.grid(axis="y")
            ax.set_xlabel("")
            ax.set_xlim([1, 32])
            ax.set_xticks(range(1, 32, 2), range(1, 32, 2))
        ...
        for ax in axes[::2]:
            ...
            ax.set_ylim([0, 3200])
        for ax in axes[1::2]:
            ax.set_yticks([0, 250, 500, 750], [0, 250, 500, 750])
            ax.set_ylim([0, 850])
        axes[0].legend(
            loc="lower center",
            bbox_to_anchor=(1.04, 1.2),
            ncol=3,
            fancybox=True,
            fontsize=21,
        )
        plt.show()
        fig.savefig(f"{MT_FINAL_FIGURES_DIR}/lfb_reliability_absolute.pdf")
